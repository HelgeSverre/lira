...
- **Fiber mode toggle — backend DONE (2026-06-25):** `Run`/`Debug` client messages carry a `#[serde(default)] fiber_mode` field; `DebugSession::set_fiber_mode` applies it. Plain `Run` goes straight to completion (`run_to_completion`); fiber-mode `Debug` is **steppable** (see concurrent step debugging below). _Remaining:_ frontend UI toggle + visualization.
